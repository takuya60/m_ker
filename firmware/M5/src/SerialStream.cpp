...
SerialStream::SerialStream(Stream& serial)
    : _serial(serial)
    , _error(false)
    , _on_command(nullptr)
    , _field_count(0)
...
bool SerialStream::add(const char* key, Type type, size_t count) {
...
void SerialStream::onCommand(CommandCallback cb) {
    _on_command = cb;
}

FieldDef* SerialStream::_find(const char* key) {
...
// set() - scalar
void SerialStream::set(const char* key, uint32_t value) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, &value, sizeof(uint32_t));
}
void SerialStream::set(const char* key, uint16_t value) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, &value, sizeof(uint16_t));
}
void SerialStream::set(const char* key, uint8_t value) {
    FieldDef* f = _find(key); if (!f) return;
    _bin_buf[f->offset] = value;
}
void SerialStream::set(const char* key, int32_t value) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, &value, sizeof(int32_t));
}
void SerialStream::set(const char* key, int16_t value) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, &value, sizeof(int16_t));
}
void SerialStream::set(const char* key, float value) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, &value, sizeof(float));
}
void SerialStream::set(const char* key, bool value) {
    FieldDef* f = _find(key); if (!f) return;
    _bin_buf[f->offset] = value ? 1 : 0;
}

// set() - array
void SerialStream::set(const char* key, const float* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, array, sizeof(float) * len);
}
void SerialStream::set(const char* key, const int16_t* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, array, sizeof(int16_t) * len);
}
void SerialStream::set(const char* key, const uint32_t* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, array, sizeof(uint32_t) * len);
}
void SerialStream::set(const char* key, const uint16_t* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, array, sizeof(uint16_t) * len);
}
void SerialStream::set(const char* key, const int32_t* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    memcpy(_bin_buf + f->offset, array, sizeof(int32_t) * len);
}
void SerialStream::set(const char* key, const bool* array, size_t len) {
    FieldDef* f = _find(key); if (!f) return;
    for (size_t i = 0; i < len; i++) _bin_buf[f->offset + i] = array[i] ? 1 : 0;
}

// send()
uint8_t SerialStream::_checksum() const {
    uint8_t cs = 0;
    for (size_t i = 0; i < _bin_total; i++) cs ^= _bin_buf[i];
    return cs;
}

bool SerialStream::send() {
    ...
    return _serial.write(out, _bin_total + 3) > 0;
}

// recv()
size_t SerialStream::recv(uint8_t* buf, size_t len) {
    size_t received = 0;
    while (_serial.available() && received < len) {
        buf[received++] = _serial.read();
    }
...
void SerialStream::sendPingResponse(const SensorSnapshot& snapshot,
...
// Utilities
size_t SerialStream::_typeSize(Type type) {
    switch (type) {
...
const char* SerialStream::_typeName(Type type) {
    switch (type) {
