...
它读取 KER 的 16 路编码器数据，将固件角度转换为 ROS 2 关节目标，再通过带低通滤波、
速度受限插值和目标超时保护的 bridge 输出到 ros2_control position controller。
...
本包不负责 MoveIt 规划、碰撞检测、加速度限制、CAN 硬件驱动或固件烧录。目标超时后
...
默认将左右 J7 的 `joint_scale` 设为 `-1.0`，用于匹配 KER 夹爪俯仰与 OpenArmX/RViz 的正方向。
...
| `pose_processor.py` | 度转弧度、scale/offset、夹爪映射、Hampel/一阶低通滤波。 |
...
固件必须提供 16 项 `angles`。处理器默认用一阶低通滤波平滑数据，也可叠加 5 样本
Hampel 窗口抑制孤立跳点，然后发布源弧度数据，并计算两侧手臂及夹爪目标。滤波不能
用于掩盖机械松动、错误 ID 或 360 度回绕。
...
bridge 按 OpenArmX 关节名读取 `/joint_states`，而不是依赖数组顺序。首次目标从机器人
当前姿态开始，之后每个控制周期向最新 KER 目标移动，但不超过
`max_joint_velocity_rad_s` 和 `max_gripper_velocity_m_s`。因此较大位置差不会拒绝接管，
而是以受限速度逐步追到目标。超过 `target_timeout_s` 没有新目标时停止发布。
...
| `use_low_pass_filter` | `true` | 是否启用一阶低通滤波。 |
| `low_pass_alpha` | `0.2` | 低通权重；越小越平滑、延迟越大。 |
| `drop_command_on_sensor_error` | `true` | 任一错误位时停止目标。 |
| `gripper_min_position` | `0.0` | 夹爪闭合位置。 |
| `gripper_max_position` | `0.044` | 夹爪打开位置。 |
| `joint_scales` | 左右 J7 为 `-1.0`，其余为 `1.0` | 手臂比例和方向。 |
| `joint_offsets` | 14 个 `0.0` | 手臂弧度偏移。 |

YAML 还可以配置源话题、错误话题、左右目标话题和三个 Ping 服务名。

编码器自身读数方向和零位应优先在固件中修正；KER 与 OpenArmX 坐标系之间的方向差异使用 ROS `joint_scales` 修正，不要在两层重复反向。
...
1. 默认任一固件错误位停止目标更新。
2. 缺少一侧任一目标名称时忽略整条消息。
3. 关节和夹爪目标始终按配置的最大速度追踪。
4. 目标超过 0.25 秒未更新时停止 bridge 发布。
5. driver 关闭时向 KER 发送 STANDBY。

限制：错误保护目前是全局的；bridge 没有正式 `right_arm_only` 参数；没有碰撞检测、
加速度限制和主动回零保护。位置差不再导致拒绝接管，因此实机启动前仍必须人工确认姿态。
